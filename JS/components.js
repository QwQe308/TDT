...
// data = a function returning the content (actually HTML)
	...
	Vue.component('main-display', {
		props: ['layer', 'data'],
		template: `
		<div><span v-if="player[layer].points.lt('1e1000')">现有 </span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px ' + tmp[layer].color}">{{data ? format(player[layer].points, data) : formatWhole(player[layer].points)}}</h2> {{tmp[layer].resource}}<span v-if="layers[layer].effectDescription">, <span v-html="run(layers[layer].effectDescription, layers[layer])"></span></span><br><br></div>
		`
	})
...
// Updates the value in player[layer][data]
	Vue.component('slider', {
		...
		template: `
			...
			<input v-on:input="player[layer][data[0]] = ($event.target.value)" type="range" v-model="player[layer][data[0]]" :min="data[1]" :max="data[2]" :value="player[layer][data[0]].toString()"></div>
		`
	})

	//Genoplex: I'd fxxking give up
	Vue.component('ES_Slider',
	    {
		    props: ['layer', 'data'],
		    template: `<input v-on:input="player.ExoticSubstances.Substance_TubePercent = ($event.target.value)" type="range" min="0" :max="100" step="1" style="width: 150px" :value="player.ExoticSubstances.Substance_TubePercent.toString()">`
	    }
	)

	Vue.component('S_Slider',
	    {
		    props: ['layer', 'data'],
		    template: `<input v-on:input="player.Substances.Throw_Percent = ($event.target.value)" type="range" min="0" :max="100" step="1" style="width: 300px" :value="player.Substances.Throw_Percent.toString()">`
	    }
	)
